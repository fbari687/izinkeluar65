        ...
        <div class="w-full flex flex-col items-center p-2.5 gap-5">
                ...
                <div class="w-6/12 flex gap-4">
                    <form action="" class="w-2/4 border bg-[#ddd] px-2 py-1 flex items-center gap-2">
                        ...
                        <input type="text" class="outline-none w-full bg-transparent text-black"
                            placeholder="Search By Nama Kelas" name="search">
                    </form>
                    <a href="/admin/kelas/create" class="flex items-center gap-2 bg-[#26577C] text-white px-2">
                        <span>ADD</span>
            ...
            @else
                <div class="w-full flex-col">
                    <div class="py-2 font-bold text-xl">Kelas</div>
                    <div class="w-full flex uppercase items-center">
                        <div class="w-1/12 border-2 border-gray-500 text-center font-bold py-2">No</div>
                        <div class="w-2/12 border-2 border-gray-500 text-center font-bold py-2">Nama Kelas</div>
                        <div class="w-2/12 border-2 border-gray-500 text-center font-bold py-2">Jurusan</div>
                        <div class="w-3/12 border-2 border-gray-500 text-center font-bold py-2">Wali Kelas</div>
                        <div class="w-2/12 border-2 border-gray-500 text-center font-bold py-2">Tingkatan</div>
                    ...
                    @foreach ($BanyakKelas as $kelas)
                        <div class="w-full flex items-center {{ $loop->iteration % 2 == 0 ? 'bg-[#ddd]' : 'bg-white' }}">
                            ...
                            <div class="w-2/12 text-center font-bold py-2">{{ $kelas->nama_kelas }}</div>
                            <div class="w-2/12 text-center font-bold py-2">{{ $kelas->jurusan->nama_jurusan }}</div>
                            <div class="w-3/12 text-center font-bold py-2">{{ $kelas->waliKelas->nama }}</div>
                            <div class="w-2/12 text-center font-bold py-2">{{ $kelas->tingkatan_kelas }}</div>
                                ...
                                <form action="/admin/kelas/{{ $kelas->id }}" method="POST"
                                    ...
                                    <div class="w-full h-full flex items-center justify-center">
                                        <div
                                            ...
                                            <span class="font-semibold text-lg">Apakah Anda Yakin Ingin Menghapus Kelas
                                                <span class="font-bold">{{ $kelas->nama_kelas }}</span></span>
