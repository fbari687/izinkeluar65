...
<section id="content" class="pt-12 w-full h-full mx-auto">
  <form action="/admin/kelas/{{ $kelas->id }}" method="POST" class="w-full flex flex-col items-center p-5 gap-2">
    @csrf
    @method('put')
    <span class="w-full text-2xl font-bold">Edit Data Kelas</span>
    <div class="w-full flex flex-col gap-y-1">
      <span class="font-bold text-lg">Nama Kelas :</span>
      <input type="text" class="w-full px-2 py-1.5 border-2 text-black rounded-md" name="nama_kelas" placeholder="Nama Kelas" value="{{ old('nama_kelas', $kelas->nama_kelas) }}">
      @error('nama_kelas')
    ...
    <div class="w-full flex flex-col gap-y-1">
      ...
      <select data-te-select-init name="jurusan_id">
        ...
        <option value="{{ $jurusan->id }}" {{ $jurusan->id == old('jurusan_id', $kelas->jurusan_id) ? 'selected' : '' }}>{{ $jurusan->nama_jurusan }}</option>
        @endforeach
      </select>
      @error('jurusan_id')
      <span class="text-red-500 text-sm px-2 font-semibold">{{ $message }}</span>
    ...
    <div class="w-full flex flex-col gap-y-1">
      ...
      <select data-te-select-init name="tingkatan_kelas">
        @for ($i = 10; $i <= 13; $i++)
          <option value="{{ $i }}" {{ $i == old('tingkatan_kelas', $kelas->tingkatan_kelas) ? 'selected' : '' }}>{{ $i }}</option>
        @endfor
      </select>
    ...
    <div class="w-full flex flex-col gap-y-1">
      ...
      <select data-te-select-init data-te-select-filter="true" name="wali_kelas_id">
        ...
        <option value="{{ $guru->user->id }}" {{ $guru->user->id == old('wali_kelas_id', $kelas->wali_kelas_id) ? 'selected' : '' }}>{{ $guru->user->nama }}</option>
        @endforeach
      </select>
      @error('wali_kelas_id')
      <span class="text-red-500 text-sm px-2 font-semibold">{{ $message }}</span>
