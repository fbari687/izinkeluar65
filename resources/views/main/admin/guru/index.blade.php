...
@section('content')
    ...
    <section id="content" class="pt-12 w-full h-full mx-auto">
        <div class="w-full flex flex-col items-center p-2.5 gap-5">
                ...
                <div class="w-6/12 flex gap-4">
                    <form action="" class="w-2/4 border bg-[#ddd] px-2 py-1 flex items-center gap-2">
                        ...
                        <input type="text" class="outline-none w-full bg-transparent text-black"
                            placeholder="Search By Nama" name="search">
                    </form>
                    <a href="/admin/guru/create" class="flex items-center gap-2 bg-[#26577C] text-white px-2">
                        <span>ADD</span>
            ...
            @if ($BanyakGuru->isEmpty())
                <div class="w-full flex-col text-center font-bold text-xl mt-2">
                    <h2>Belum Ada Guru</h2>
                </div>
            @else
                <div class="w-full flex-col">
                    <div class="py-2 font-bold text-xl">Guru</div>
                    <div class="w-full flex uppercase items-center">
                        ...
                        <div class="w-2/12 border-2 border-gray-500 text-center font-bold py-2">Username</div>
                        <div class="w-1/12 border-2 border-gray-500 text-center font-bold py-2">Wali Kelas</div>
                        <div class="w-1/12 border-2 border-gray-500 text-center font-bold py-2">Guru Piket</div>
                        <div class="w-1/12 border-2 border-gray-500 text-center font-bold py-2">Guru Matpel</div>
                        <div class="w-1/12 border-2 border-gray-500 text-center font-bold py-2">Kaprodi</div>
                        <div class="w-2/12 border-2 border-gray-500 text-center font-bold py-2">Action</div>
                    </div>

                    @foreach ($BanyakGuru as $user)
                        <div class="w-full flex items-center {{ $loop->iteration % 2 == 0 ? 'bg-[#ddd]' : 'bg-white' }}">
                            ...
                            <div class="w-3/12 text-center font-bold py-2">{{ $user->nama }}</div>
                            <div class="w-2/12 text-center font-bold py-2">{{ $user->username }}</div>
                            <div class="w-1/12 text-center font-bold py-2">
                                @if ($user->guru->wali_kelas == true)
                            ...
                            <div class="w-2/12 text-center font-bold py-2 grid grid-cols-2 gap-1">
                                <a href="/admin/guru/{{ $user->id }}/edit" class="bg-[#26577C] text-white rounded-md"><i
                                ...
                                <form action="/admin/guru/{{ $user->id }}" method="POST"
                                    ...
                                    <div class="w-full h-full flex items-center justify-center">
                                        <div
                                            ...
                                            <span class="font-semibold text-lg">Apakah Anda Yakin Ingin Menghapus Guru <span
                                                    class="font-bold">{{ $user->nama }}</span></span>
