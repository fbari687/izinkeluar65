            ...
            <div class="w-full flex items-center">
                <div class="w-6/12 flex gap-4">
                    <form action="" class="w-2/4 border bg-[#ddd] px-2 py-1 flex items-center gap-2">
                        ...
                        <input type="text" name="search" class="outline-none w-full bg-transparent text-black"
                            placeholder="Search By NIS">
                    </form>
                    <a href="/admin/siswa/create" class="flex items-center gap-2 bg-[#26577C] text-white px-2">
                        <span>ADD</span>
                ...
                <div class="w-full flex-col">
                    ...
                    @foreach ($BanyakSiswa as $siswa)
                        <div class="w-full flex items-center {{ $loop->iteration % 2 == 0 ? 'bg-[#ddd]' : 'bg-white' }}">
                            ...
                            <div class="w-2/12 text-center font-bold py-2">{{ $siswa->nis }}</div>
                            <div class="w-3/12 text-center font-bold py-2">{{ $siswa->user->nama }}</div>
                            <div class="w-2/12 text-center font-bold py-2">{{ $siswa->kelas->tingkatan_kelas }}</div>
                            <div class="w-2/12 text-center font-bold py-2">{{ $siswa->jurusan->nama_jurusan }}</div>
                            <div class="w-2/12 text-center font-bold py-2 grid grid-cols-2 gap-1">
                                <a href="/admin/siswa/{{ $siswa->user->id }}/edit"
                                ...
                                <form action="/admin/siswa/{{ $siswa->user->id }}" method="POST"
                                    ...
                                    <div class="w-full h-full flex items-center justify-center">
                                        <div
                                            ...
                                            <span class="font-semibold text-lg">Apakah Anda Yakin Ingin Menghapus Siswa
                                                <span class="font-bold">{{ $siswa->user->nama }}</span></span>
