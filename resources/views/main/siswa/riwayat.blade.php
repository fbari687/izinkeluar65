        ...
        <div class="w-full flex flex-col items-center p-2.5 gap-5">
            ...
            <div class="w-full flex flex-col gap-2">
                @foreach ($banyakPerizinanSiswa as $perizinanSiswa)
                    <a href="/siswa/{{ $perizinanSiswa->id }}/detail"
                        class="bg-[#4A4A4A] p-2.5 w-full flex flex-col gap-2 rounded-md transition duration-150 hover:bg-[#292929]">
                        <div class="w-full flex justify-start items-center text-white font-bold">
                        ...
                        @endphp
                            ...
                            {{ $filteredKeterangans->last()->keterangan_perizinan_id }}</span>
                        <span class=" text-white font-bold text-sm">STATUS: <span
                                class="{{ $perizinanSiswa->status_perizinan_id == 3 ? 'text-green-500' : 'text-[#FAFF01]' }} uppercase">{{ $perizinanSiswa->statusPerizinan->nama }}</span></span>
                    </a>
                @endforeach
            </div>
        </div>
