...
@section('content')
  <section id="content" class="pt-12 w-full h-full">
    <a href="/guru/riwayat" class="w-fit px-4 py-6 flex items-center justify-start self-start"><i class="fa-solid fa-arrow-left-long fa-2xl"></i></a>
    <div class="w-full h-full flex flex-col gap-5 px-3 py-3">
      ...
      <div class="w-full flex flex-col gap-3">
        ...
        <div class="w-full flex flex-col rounded-lg overflow-hidden">
          ...
          <div class="w-full flex bg-[rgb(73,73,73)] text-white font-bold text-sm">
            <div class="w-2/12 text-center border-[0.2px] border-black p-2">{{ $perizinan->keterangan_perizinan_id }}</div>
        ...
        <div class="w-full flex flex-col rounded-lg overflow-hidden">
          ...
          <div class="w-full flex bg-[rgb(73,73,73)] text-white font-bold text-sm">
            @if ($perizinan->keterangan_perizinan_id == 11)
            <div class="w-9/12 text-center border-[0.2px] border-black p-2">{{ $perizinan->guru->nama }}</div>
            <div class="w-3/12 text-center border-[0.2px] border-black p-2">{{ $perizinan->statusPerizinan->nama }}</div>
            <input type="hidden" hidden name="keterangan_perizinan_id[]" value="{{ $perizinan->keterangan_perizinan_id }}">
            @endif
          </div>
          @endforeach
        ...
        <div class="w-full flex flex-col rounded-lg overflow-hidden">
          ...
          <div class="w-full flex bg-[rgb(73,73,73)] text-white font-bold text-sm">
            ...
            <div class="w-9/12 text-center border-[0.2px] border-black p-2">{{ $perizinan->guru->nama }}</div>
            <div class="w-3/12 text-center border-[0.5px] border-black p-2">{{ $perizinan->statusPerizinan->nama }}</div>
            <input type="hidden" hidden name="keterangan_perizinan_id[]" value="{{ $perizinan->keterangan_perizinan_id }}">
            @endif
          </div>
          @endforeach
